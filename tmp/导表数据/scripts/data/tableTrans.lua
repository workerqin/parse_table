table.print = function( tb )
	...
	local function print_table(tb_data)
		...
		if type(tb_data) ~= "table"  then
			cclog("Error", "存储类型必须为table:", tb )
			return
		end
		if tb_cache[tb] then
			cclog("Error", "无法继续存储，table中包含循环引用，", tb )
			return
		end
		local k, v
		cur_deep = cur_deep + 1
 ...
 table.save_fd = function(file, tb, tb_deep, func_save_item)
	 ...
	 local function save_table(tb_data)
		 -- 存储当前层table
		 if type(tb_data) ~= "table"  then
			 cclog("Error", "存储类型必须为table:", tb, path, tb_deep)
			 return
		 end
		 if tb_cache[tb] then
			 cclog("Error", "无法继续存储，table中包含循环引用，", tb, path, tb_deep)
			 return
		 end
		 local k, v
		 cur_deep = cur_deep + 1
		 if cur_deep > tb_deep then
			 cclog("Error", "待存储table超过可允许的table深度", tb, path, tb_deep)
			 cur_deep = cur_deep -  1
			 return
		 end
		 local tab = string.rep(" ", (cur_deep-1)*4)
		 ...
		 for i, k in ipairs(keys) do
			 ...
			 if type(k) == "boolean" then
      		...
      		if type(v) == "number" then
        		local integer,decimal=math.modf(v)
       	 		if math.abs(decimal)>0.0001 then
          			value = string.format("%f", v)
        		else
 ...
 table.save_fd2 = function(file, tb, tb_deep, func_save_item)
	 if type(tb) ~= "table" or not file then		
		 return
	 end
 ...
 -- 地图专用
 table.save_map = function(tb, path, tb_deep, is_compile, is_compress, func_save_item)
	 ...
	 -- table.save_fd参数依次为：file, tb, tb_deep, func_save_item
	 local rtn = table.save_fd2(file, tb, tb_deep, func_save_item)
	 file:close()
	 return rtn
 end
