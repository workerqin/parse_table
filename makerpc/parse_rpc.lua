...
function parse_one(text, filename)
...
function parser.parse_file(filename)
	...
	f:close()
	return parse_one(buffer,filename)
end
...
function parser.parse_dir(path)
	...
	for _, file in ipairs(listfiles) do
		...
		local buffer = f:read "*a"
		f:close()
		local one_res = parse_one(buffer, file)
		table.insert(all, {
			["file"] = file, 
			["content"] = one_res})
	end
...
function parser.server_file_lpc_serial(content, filename)
	...
	local serial_func_table = {
		...
		["function"] = function(e)
			...
			table.insert(args_name_tbl, 2, string.format("%s", string.upper(e.name)))
