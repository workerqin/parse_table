...
#int变float
to_float_pattern = re.compile("(\d{1,9}[.]\d{1,9}|\d{1,9}|[\+\-\*\/\(\)]|＋|,|\"|\W|\w)")
def formla_int_to_float(formla):
    result = ""
    values = to_float_pattern.findall(formla)
    for data in values:
        if data.isdigit() and data.find(u".") < 0:
            result += data + u".0"
        else:
            if data == u"＋":
                result += u"+"
