...
def parse_function(val):
	if not len(val) :
		return 0
	return "@@fun (mapping env) {return (%s);}" % (val)

def parse_function_null(val):
	if not len(val) :
		return 0
	return "@@fun (mapping env, mapping ret) {%s}" % (val)
...
def do_parse_value(type, val):
	...
	elif type == "string" or type == "str" or type == "text":
	...
	elif type == "function_null":
		#自定义的函数
		return parse_function_null(val)
	elif type == "mixed":
...
def DoParseSheet(dictConfig, SheetDict, to_lpc):
	...
	if "udata" in dictConfig.keys():
		for dkey in dictConfig["udata"].keys():
			...
			if to_lpc:
				vars_map["udata"][dkey] = Util.PythonData2Lpc(parse_str)
			else:
				vars_map["udata"][dkey] = parse_str
	...
	for row in range(parserow, SheetDict["nrows"], 1): 
		...
		for key in vars_col["Cols"].keys():
			value = vars_col["Cols"][key]
			if to_lpc:
				vars_col["Cols"][key] = Util.PythonData2Lpc(value)
			else:
				vars_col["Cols"][key] = value
	...
	for row in range(0, SheetDict["nrows"], 1):
		...
		if value == "表项信息":
			...
			for parserow in range(row + 1, SheetDict["nrows"], 1):
				...
				if dictkey == "End" or dictkey == "end":
					currow = parserow
					break
				if dictkey == "Begin" or dictkey == "begin":
					begin = 1
					continue
				if not begin:
					continue
	...
	for row in range(currow + 1, SheetDict["nrows"], 1): 
		...
		if value in dictConfig.keys():
			...
			if not isinstance(types, list):
				...
				vals = []
				for i in range(0, len(types), 1):
					if types[i].find(":") >= 0 and types[i].split(":")[1] == "primary":
						continue
					currow = 1+i
...
def DoParseNormalWrite(dictConfig, ParseFile, ParseSheet, OutputFile, SheetDict = {}):
	...
	vars_map = DoParseSheet(dictConfig, SheetDict, 1)
...
def DoParseNormal(dictConfig, ParseFile, ParseSheet):
	SheetDict = XlsSheet2Dict(ParseFile, ParseSheet)
	return DoParseSheet(dictConfig, SheetDict, 1)

def DoParseNormal2(dictConfig, ParseFile, ParseSheet):
	SheetDict = XlsSheet2Dict(ParseFile, ParseSheet)
	parsedata = DoParseSheet(dictConfig, SheetDict, 0)
	res = {}
